...
namespace TrackTimeSpendInProjectAndroidAapp
{
    public class Members : SupportFragment
    {
        ...
        public async Task UpdateMemebers()
        {
            ...
            {
                Application.SynchronizationContext.Post(_ => { Button.Visibility = ViewStates.Visible; }, null);
                ...
                while (true)
                {
                    ...
                    if (models != null && models.Count > 0)
                    {
                        foreach (var x in models)
                        {
                            try
                            {
                                if (MemebersModelsList.First(y => y.Id == x.Id).Active != x.Active)
                                {
        ...
        public class RecyclerAdapter : RecyclerView.Adapter
        {
            ...
            public override void OnBindViewHolder(RecyclerView.ViewHolder holder, int position)
            {
                if (holder is MemberRCModel)
                {
                    ...
                    myHolder.mCurrentTask.Text = "Siaip";
                    myHolder.mCurrentActiveTime.Text = "8H - 23M";
                    myHolder.mTotalTime.Text = "23H - 2H - 30M";
